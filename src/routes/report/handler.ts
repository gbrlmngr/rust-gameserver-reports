...
const post = async (request: IRequest, env: Env, ctx: ExecutionContext): Promise<Response> => {
  try {
    if (
      !env.REPORTS_DISCORD_WEBHOOK_URL
      || !env.REPORTS_DISCORD_WEBHOOK_URL.startsWith('https://discord.com/api/webhooks/')
    ) {
...
export const handler = Router({ base: '/report' })
  .post('/', post)
  .handle;
